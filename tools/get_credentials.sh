...
PROJECT_ID=`cat $CLIENT_SECRET_FILE | grep -o -e '"project_id":"[^"]*"' | sed -e 's/.*"project_id":"\([^"]*\)".*/\1/g'`
...
ACCESS_TOKEN=`cat $CREDENTIALS_FILE | grep -o -e '"access_token": "[^"]*"' | sed -e 's/"access_token": "\([^"]*\)"/\1/g'`
REFRESH_TOKEN=`cat $CREDENTIALS_FILE | grep -o -e '"refresh_token": "[^"]*"' | sed -e 's/"refresh_token": "\([^"]*\)"/\1/g'`
TOKEN_TYPE=`cat $CREDENTIALS_FILE | grep -o -e '"token_type": "[^"]*"' | sed -e 's/"token_type": "\([^"]*\)"/\1/g'`
if [ -z $ACCESS_TOKEN ]; then
  echo "Failed to get access token. Exiting. OAuth server response:"
